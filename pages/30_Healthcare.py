...
with st.form("health_quiz"):
    profile = st.selectbox(t("profile"), [t("student"),t("worker"),t("family"),t("retiree")])
    ssn = st.radio("Numéro de sécu ?", [t("yes"),t("no")], horizontal=True)
    submitted = st.form_submit_button(t("see_rec"))
if submitted:
    steps = []
    if ssn == t("no"): steps.append("Demander un numéro de sécurité sociale (CPAM).")
